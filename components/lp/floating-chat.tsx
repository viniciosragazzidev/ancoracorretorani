...
const consultantMessages = [
    { name: "Luciana", text: "Luciana está online e tem um desconto pra você!", avatar: "👩‍💼" },
    { name: "Vanessa", text: "Vanessa liberou tabela PME com 35% de desconto!", avatar: "👩‍⚕️" },
    { name: "Amanda", text: "Amanda tem cotações sem carência prontas para você!", avatar: "👩‍💻" },
];
...
const FloatingChat: React.FC = () => {
    ...
    // Rotate toast messages every 8 seconds
    useEffect(() => {
        const interval = setInterval(() => {
            setCurrentMsgIndex((prev) => (prev + 1) % consultantMessages.length);
        }, 8000);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (cleaned.length >= 10 && nome) {
            ...
            const consultant = consultantMessages[currentMsgIndex].name;
            setTimeout(() => {
                setIsSubmitting(false);
                setIsOpen(false);
                const msg = `Olá ${consultant}! Me chamo ${nome} e gostaria de aproveitar o desconto exclusivo via ${tipo.toUpperCase()}.`;
                window.open(`https://wa.me/5521974450263?text=${encodeURIComponent(msg)}`, '_blank');
            }, 600);
        }
    };

    const activeConsultant = consultantMessages[currentMsgIndex];

    return (
        <div className="fixed bottom-5 right-5 z-[80] font-sans flex flex-col items-end select-none">
            
            {/* 1. CHAT MODAL FLUTUANTE (ABERTO) */}
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="mb-4 w-[calc(100vw-32px)] max-w-[380px] bg-background/95 dark:bg-background/95 backdrop-blur-2xl border border-border/60 rounded-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] relative overflow-hidden"
                    >
                        {/* Internal Top Gradient Glow */}
                        <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-blue-500 via-emerald-500 to-blue-500 animate-gradient" />
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col text-left">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-500/10 px-1.5 py-0.5 rounded-full">
                                            Online
                                        </span>
                                    </div>
                                    <span className="text-[11px] text-muted-foreground font-light">
                                        Consultora Âncora Corretora
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className="p-5 text-left space-y-4">
                            <div className="bg-primary/5 border border-primary/10 rounded-2xl p-3 text-xs text-foreground font-medium leading-relaxed flex items-start gap-2">
                                ...
                                <span>
                                    Olá! Sou a <strong className="text-primary font-bold">{activeConsultant.name}</strong>. Preencha abaixo e receba sua cotação com desconto em segundos!
                                </span>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-3.5">
                                ...
                                >
                                    {isSubmitting ? (
                                        ...
                                        <>
                                            <span>Falar com {activeConsultant.name} Agora</span>
            ...
            <AnimatePresence>
                {!isOpen && showToast && (
                    ...
                    >
                        <div className="size-8 rounded-full bg-emerald-500/10 text-base flex items-center justify-center shrink-0 border border-emerald-500/20">
                            {activeConsultant.avatar}
                        </div>
            ...
            <motion.button
                ...
                className={`relative size-14 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white shadow-2xl flex items-center justify-center transition-all cursor-pointer border-2 border-white/20 ${
